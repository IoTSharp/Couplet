...
C0 已完成，C1 处于实现中：CPL-010~012 已落地；CPL-013~015 在 source lane 已接通 active generation 原子切换、planning snapshot、query lease/cursor、writer fence、cutoff-aware cleanup、确定性提交边界故障重开、`workspace_status`、`code_search` exact/fulltext Preview、fulltext 同 active cursor/no-scan、path/language/entity-kind 过滤计划与 `symbol_get` 小型回归，`CG-005` 进入 verifying，`CG-007` 的 API/接线缺口已关闭。retired generation 跨请求续页、实时 watcher freshness、真实子进程 publish 前后 kill/reopen、Medium/Large 增量与双客户端门禁仍未通过，C1 不能标完成。默认 3.1.0 package 的 Native AOT 仍受 CG-006 限制；source lane 已使用修复后的 worker 路径，本轮 CLI、Daemon 与 MCP Server 的 win-x64 source publish 均无未处置 IL/AOT warning，但 7 天长稳仍待归档。C2 Preview、C3 Beta 和 C4 Production 继续受联合门禁约束。
