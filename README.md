...
> C0 基础与合同已于 2026-08-25 完成。C1 已实现工作区/Git 发现、C#/TypeScript/JavaScript partial 适配器、增量规划和 SonnetDB Document/FullText staging。默认独立构建仍固定 `SonnetDB.Core 3.1.0` 并保持查询 unavailable；显式 `UseSonnetDbSource=true` 联调最新 SonnetDB 源码时，`index-stage` 已接入 `Tsdb.Generations` 的原子发布、active/retained query lease、cutoff-aware cleanup 和真实子进程 commit 前后 kill/reopen，显式数据库 MCP 宿主已接通 `workspace_status`、`code_search` exact/fulltext Preview、fulltext 同进程跨请求 retired-generation cursor/no-scan 与 `symbol_get`。进程重启续页、实时 watcher、双客户端、Medium/Large 容量与长稳门禁仍未完成。
...
`workspace-scan` 只输出 workspace-relative path 和去凭证仓库身份。默认 package lane 的 `index-stage` 继续只写 generation 独立 staging collection，并返回 `published=false` / `CG-005`。source lane 额外把不含源码正文的 planning snapshot、Document collection 与 FullText index 交给 `Tsdb.Generations` 原子发布；无变化且查询 schema 完整时复用 active generation，缺少当前所需索引时发布新 ordinal generation。`--retired-generation-retention <c>` 接受 invariant `TimeSpan`，表示 retired generation 从发布时间起达到该时长后才具备清理资格；默认零值保持立即清理，active 或仍有 query lease 的 generation 不会删除。source 报告使用 `couplet.index_stage.v2`，分别返回 lease-deferred 与 retention-deferred revisions；冻结的默认 package `couplet.index_stage.v1` payload/schema 不变。writer fence 覆盖锁内 fresh discovery、active read、build、plan 和 publish；发布已提交后的 cleanup 失败不会改写为发布失败，而是返回 retry limitation。除进程内确定性故障点外，自动化现在还会启动真实 `Couplet.Cli` 子进程，在 commit 前后收到测试握手后强杀，并用新进程/新 store 验证只暴露完整旧或新 revision；这是本机提交边界恢复证据，不代表 7 天或固定硬件故障门禁。`c1-capacity` 仍是未发布 staging characterization，两档 Performance/Capacity gate 均为 FAIL。固定 3.1.0 package 的 win-x64 Native AOT 仍关闭不兼容 worker 并报告 CG-006；最新 source lane 已修复 worker 路径，2026-08-29 CLI 首次 publish/no-op runtime smoke 已通过，本轮 CLI、Daemon 与 MCP Server 的 win-x64 source publish 均无未处置 IL/AOT warning，7 天长稳仍未归档，JIT handshake 也不把 source 选择外推为 AOT 已验证。

source lane 的 MCP Server 在同时显式提供 `--workspace` 与 `--database` 时，用真实 workspace discovery identity 打开数据库，并在完整 stdio host 生命周期内持有 store。`workspace_status` 每次请求获取并释放一个 active generation lease，只读取同一 planning/manifest snapshot，校验资源角色/名称、schema、identity/state 和 checksum 形状；它不会扫描 Document 行、重算 checksum 或触发 rebuild。返回的 source revision 与 database bytes 是 MCP 启动时采样值，freshness/diagnostics 会明确该边界，`RebuildRequired` 仅相对 initialize 快照判断。`code_search` exact 使用 `by_stable_id` path index；fulltext 使用 generation-bound `code_search` FullText index，path/language/entity-kind 过滤通过有界 planning snapshot、`by_path` / `by_language` / `by_entity_kind` 候选交集和 posting-stage filtered search 执行。planning/filter/posting 共用访问预算，耗尽时不返回部分结果。`symbol_get` 按 stable ID 或 qualified identity 使用 `by_stable_id` / `by_qualified_identity`，缺少 language 的歧义请求 fail closed。查询以 active 或 retained generation lease 返回 source-generated typed 响应、source evidence、实际 access path 和预算诊断，不执行 Document 全扫。fulltext `code_search` 结果截断时返回绑定完整过滤 query shape、generation/revision、little-endian offset 和 nonce 的 opaque cursor；同进程内每条链使用默认两分钟绝对 TTL、最多 128 条的 retained lease，即使 active generation 已切换也能跨请求读完旧 revision。cursor 一次性转移，并在最终页、取消、错误、到期、容量拒绝或 store dispose 时释放；主动 timer 防止 idle lease 超期，容量在 fulltext 查询前 fail fast，exact 不占 cursor slot。当前 FullText 没有原生 search-after，深页仍以预算约束的 Top-K + offset 执行，超过候选预算直接失败而不降级为 Document scan。默认 package lane、`symbol_get` cursor 和进程重启后的续页仍 unavailable；CG-005 保持 verifying，CG-007 的选择性 cleanup API/接线缺口已关闭。
...
C0 已完成，C1 处于实现中：CPL-010~012 已落地；CPL-013~015 在 source lane 已接通 active generation 原子切换、planning snapshot、active/retained query lease、writer fence、cutoff-aware cleanup、真实子进程 commit 前后 kill/reopen、`workspace_status`、`code_search` exact/fulltext Preview、fulltext 同进程跨请求 retired cursor/no-scan、path/language/entity-kind 过滤计划与 `symbol_get` 小型回归，`CG-005` 保持 verifying，`CG-007` 的 API/接线缺口已关闭。进程重启后的 cursor 恢复、实时 watcher freshness、Medium/Large 增量/容量与双客户端门禁仍未通过，C1 不能标完成。默认 3.1.0 package 的 Native AOT 仍受 CG-006 限制；source lane 已使用修复后的 worker 路径，本轮 CLI、Daemon 与 MCP Server 的 win-x64 source publish 均无未处置 IL/AOT warning，但 7 天长稳仍待归档。C2 Preview、C3 Beta 和 C4 Production 继续受联合门禁约束。
