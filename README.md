...
C0 已完成，C1 处于实现中：source lane 已接通 filter-aware revision provenance、daemon watcher、generation 原子切换/cleanup、真实 CLI commit 边界、status/search/symbol 查询和 orderly store reopen 的 durable retired cursor。最终 source `171/171`、package `90/90`，三个 win-x64 source Native AOT publish 与原生 `version` smoke 均通过。`CG-005` 仍为 verifying，C1 Correctness/Recovery 与 Performance/Capacity 均为 FAIL；真实进程重启/跨进程 cursor、hard-kill CAS、真实双客户端、跨平台、随机故障、固定硬件 Medium/Large 与 7 天长稳仍未验证。C2 Preview、C3 Beta 和 C4 Production 继续受联合门禁约束。
