...
> C0 基础与合同已于 2026-08-25 完成。C1 已实现工作区/Git 发现、C#/TypeScript/JavaScript partial 适配器、增量规划和 SonnetDB Document/FullText staging。默认独立构建仍固定 `SonnetDB.Core 3.1.0` 并保持查询 unavailable；显式 `UseSonnetDbSource=true` 联调最新 SonnetDB 源码时，`index-stage` 已接入 `Tsdb.Generations` 的原子发布、active lease/cursor、cutoff-aware cleanup 和提交边界故障回归，显式数据库 MCP 宿主已用 per-request active lease 接通 `workspace_status` 及 `code_search` exact/fulltext Preview 首切片。`symbol_get`、查询 cursor、真实子进程 kill 与容量门禁仍未完成。
