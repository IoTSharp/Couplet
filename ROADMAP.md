...
| Couplet 可执行产品 | 🚧 C1 实现中 | source generation 原子发布、cutoff-aware cleanup、提交边界故障重开和 MCP exact/fulltext/`symbol_get` 查询已接线；`code_search` fulltext 同 active generation cursor 已接通且无 Document scan，跨 generation 续页、真实进程 kill、capacity 与 7 天长稳仍受门禁约束，默认 package AOT 仍受限 |
...
- **CPL-014**：🚧 已实现 generation 独立的 SonnetDB Document/FullText staging、path/fulltext index 校验和实际访问路径探针；source lane 将 planning KV、Document 与 FullText 原子发布，并在显式数据库 MCP 宿主中用单一 per-request active lease 提供 typed `workspace_status`、`code_search` exact/fulltext Preview 与 `symbol_get`。exact / stable symbol 命中 `by_stable_id`，qualified identity 命中 `by_stable_id` 或有界 `by_qualified_identity`，fulltext 命中 generation-bound FullText index，均返回实际 access path 与有界诊断；fulltext `code_search` 已签发绑定 query shape/generation/revision/offset 的 cursor，同 active generation 可续页且不使用 Document scan。fulltext filter plan、`symbol_get` cursor 和跨 generation 续页仍 unavailable。
- **CPL-015**：🚧 已实现解析失败报告、取消、completion marker、checkpoint retry、staging consistency/reopen，以及 source lane publish/reopen、generation-bound cursor、writer fence、lease-aware/cutoff-aware retired cleanup、cleanup failure 隔离、publish 提交边界故障回归和 `workspace_status` 重开/revision selector fail-closed 回归。`--retired-generation-retention` 把生命周期时长传给可控 UTC cutoff，mixed-age 重开只删除到期且无 lease 的 revision；零值保持立即清理。`code_search` cursor 在每次请求内持有 active lease，active 未切换时连续分页；新 generation 发布后旧 cursor 稳定返回 `stale_revision/query_cursor_stale`。CG-007 的 Core API/接线缺口已关闭。真实子进程 kill-before/after-publish、retired generation 跨请求续页和容量回归未完成。
