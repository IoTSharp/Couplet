...
- 增加仓库内 Codex/Claude Code client-profile/合同 fixture gate，证明未发布 staging 不进入 `workspace_status`、`code_search` 或 `symbol_get`；该自动化不启动真实 Codex 或 Claude Code 客户端进程。
...
- source lane fulltext `code_search` 接通 path/language/entity-kind 过滤计划：generation schema 新增 `by_language` 与稀疏 `by_entity_kind`，path glob 以有界 planning snapshot 匹配后走 `by_path`，各索引候选求交后交给 SonnetDB posting-stage filtered search；planning、filter candidate 与 posting visit 共用单次查询预算，耗尽时返回稳定 `budget_exhausted` 且不泄漏部分命中。access path 显式报告 planning snapshot、各 path index 与 `document_fulltext_filtered`，cursor 绑定完整过滤 shape；旧 active generation 缺新索引时不再 no-op 复用，而是发布新 ordinal generation。CG-005 仍为 verifying，真实进程重启/跨进程 cursor、固定硬件 capacity 和双客户端证据未完成。
...
- source lane store 在 `Tsdb.Open` 前通过 database-root `.couplet-store.lock` 获取单一 live owner，构造失败与正常 dispose 均释放 ownership；本机并发 open race 和 Windows `\\?\` alias 回归证明同一物理 root 只产生一个 owner。cursor 恢复清理对可解码 record 先以 observed version CAS 写入 terminal `Claimed`；malformed 或未知版本 record 保留原 bytes，仅以 CAS 作为 version fence；随后 snapshot、delete 并再次 snapshot。两个持久化窗口故障及 orderly reopen 后均拒绝 abandoned replay、停止保留 retired revision，并允许后续 `CleanupRetired` 删除。该结果不包含独立进程、Linux/macOS/NFS/SMB 锁或 cursor hard-kill CAS 证据；未来 live backup 必须显式排除 lock file。
...
- 最新 SonnetDB source lane 启用已修复的 Native AOT background worker 生命周期；只有默认 3.1.0 package 的 AOT 路径继续禁用 worker 并报告 CG-006。普通 source/JIT handshake 不再把选择源码依赖外推为 AOT 已验证；2026-08-31 使用当前 SonnetDB 工作树重新发布 source CLI、Daemon 与 MCP Server 的 win-x64 Native AOT，均为 0 个未处置 IL/AOT warning，三个原生 executable 的 `version` smoke 均退出 0。该结果不包含生产 index/watch/MCP journey 或 7 天长稳；source runtime 的 retention cutoff 已接线，默认零值保持原立即清理行为。
