...
| generation/cursor/recovery | CPL-002、CPL-013~015 | 最新源码 `Tsdb.Generations` 的 atomic publish、query lease、cursor 与 cutoff cleanup | source ProjectReference 及 exact/fulltext/`symbol_get` 查询已可调用，CG-007 API/接线已关闭 | query cursor、真实 fault/capacity 关闭 CG-005 后 C1 PASS |
...
- source lane 的 `generation.atomic_publish`、cutoff cleanup 与 exact/fulltext/`symbol_get` active query 已有 Couplet runtime 小型接线；release level 仍被 query cursor、真实 fault/capacity 的 CG-005 联合门禁阻塞。`hybrid.shared_plan` 继续由 CG-002 阻塞。
...
- `code_search` Preview 实际走 `generation_active_lease:document_path_index:by_stable_id` 与 `generation_active_lease:document_fulltext:code_search`；`symbol_get` 走 generation-bound `by_stable_id` 或最多读取两条的 `by_qualified_identity`，并通过公开 MCP typed 响应暴露 source evidence 与诊断。它们不替代尚缺的 cursor、真实 fault/capacity 或双客户端发布门禁。
