...
| CG-005 | 最新 SonnetDB `Tsdb.Generations` 已可联调；Couplet 尚缺跨请求 retired-generation query lease、真实进程故障和容量联合门禁 | SonnetDB M40 generation contract + Couplet CPL-013/CPL-015 | Couplet C1 发布及后续 | verifying | source lane 可执行 fenced publish/lease/cursor/cutoff cleanup、`workspace_status`、exact/fulltext Preview、fulltext 同 active cursor/no-scan、path/language/entity-kind 过滤和 `symbol_get`；默认 package 仍仅 staging，跨 generation cursor 继续 unavailable，不引入第二提交日志 |
...
- 实际：固定 package 只能完成 staging。source lane 已将 planning KV、Document 与 FullText 交给 `Tsdb.Generations`，小型回归覆盖 publish/reopen/no-op、active lease、cursor stale、writer fence、lease/cutoff-aware cleanup、publish 后 cleanup failure 隔离、不扫描 Document 的 `workspace_status`，以及 exact stable-ID/path-index、fulltext、fulltext 同 active cursor/no-scan、path/language/entity-kind 过滤和 `symbol_get` active-lease MCP 查询。过滤计划通过有界 planning snapshot path glob、`by_path` / `by_language` / `by_entity_kind` 候选交集和 SonnetDB posting-stage filtered search 执行，planning/filter/posting 共用预算。cursor 只保证 active 未切换时连续分页；当前 Core API 不能按 cursor 重新租用 retired generation，因此新 generation 发布后旧 cursor 显式 stale。实时 watcher freshness 和 publish 前后真实进程故障尚未接线。既有 Medium/Large 数据来自全量 staging，不能外推 source generation 性能。
...
- 关闭证据：已有 SonnetDB public API/Core 回归和 Couplet 小型 cursor/lease/cutoff cleanup、`workspace_status`、exact/fulltext Preview、fulltext 同 active cursor/no-scan、path/language/entity-kind 过滤与 `symbol_get` 回归；仍需 kill-before/after-publish、实时 watcher、跨 generation cursor 决策/证据、Codex/Claude Code 发布后查询一致性及 Medium/Large 固定硬件双门禁共同 PASS
