...
| CG-005 | 最新 SonnetDB `Tsdb.Generations` 已可联调；Couplet 已补同进程 retired-generation query lease 与真实子进程 commit 边界，仍缺进程重启 cursor、实时 watcher、双客户端和容量联合门禁 | SonnetDB M40 generation contract + Couplet CPL-013/CPL-015 | Couplet C1 发布及后续 | verifying | source lane 可执行 fenced publish/active-retained lease/cursor/cutoff cleanup、真实子进程 kill/reopen、`workspace_status`、exact/fulltext Preview、path/language/entity-kind 过滤和 `symbol_get`；默认 package 仍仅 staging，进程重启 cursor 继续 unavailable，不引入第二提交日志 |
...
- 实际：固定 package 只能完成 staging。source lane 已将 planning KV、Document 与 FullText 交给 `Tsdb.Generations`，小型回归覆盖 publish/reopen/no-op、active/retained lease、writer fence、lease/cutoff-aware cleanup、publish 后 cleanup failure 隔离、不扫描 Document 的 `workspace_status`，以及 exact stable-ID/path-index、fulltext、path/language/entity-kind 过滤和 `symbol_get` MCP 查询。fulltext cursor 以默认两分钟绝对 TTL、128 slot 和一次性 nonce 在同进程跨请求保留 retired generation；active 切换后旧 cursor 可继续，主动 timer 与所有终态释放 lease，进程重启后 fail closed。真实 CLI 子进程 commit 前/后强杀与重开分别只暴露完整旧/新 generation，后续真实 retry/no-op 完成一致性和 cleanup。实时 watcher freshness、双客户端、进程重启 cursor 决策及固定硬件容量仍未完成；既有 Medium/Large 数据来自全量 staging，不能外推 source generation 性能。
...
- 关闭证据：已有 SonnetDB public API/Core 回归和 Couplet active/retained cursor/lease/cutoff cleanup、真实子进程 commit 前后 kill/reopen、`workspace_status`、exact/fulltext Preview、path/language/entity-kind 过滤与 `symbol_get` 回归；仍需实时 watcher、进程重启 cursor 决策/证据、Codex/Claude Code 发布后查询一致性及 Medium/Large 固定硬件双门禁共同 PASS
...
- 实际：固定 3.1.0 package 仍复现并使用禁用 worker 的显式限制路径；最新 SonnetDB source 已修复 worker shutdown 并通过 Core win-x64 Native AOT 三 worker实跑。Couplet source win-x64 Native AOT CLI 首次 generation publish 与 unchanged no-op 均退出 0、无 CG-006 limitation；本轮 CLI、Daemon、MCP Server 三个 source publish 也均无未处置 IL/AOT warning，7 天长稳尚待归档。
