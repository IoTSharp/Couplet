...
| CG-005 | 最新 SonnetDB `Tsdb.Generations` 已可联调；Couplet 尚缺完整 MCP active query、真实进程故障和容量联合门禁 | SonnetDB M40 generation contract + Couplet CPL-013/CPL-015 | Couplet C1 发布及后续 | verifying | source lane 可执行 fenced publish/lease/cursor/cutoff cleanup、`workspace_status`、exact/fulltext Preview、fulltext 同 active cursor/no-scan 和 `symbol_get`；默认 package 仍仅 staging，跨 generation cursor 继续 unavailable，不引入第二提交日志 |
...
- 关闭证据：已有 SonnetDB public API/Core 回归和 Couplet 小型 cursor/lease/cutoff cleanup、`workspace_status`、exact/fulltext Preview、fulltext 同 active cursor/no-scan 与 `symbol_get` 回归；仍需 kill-before/after-publish、实时 watcher、跨 generation cursor 决策/证据、Codex/Claude Code 发布后查询一致性及 Medium/Large 固定硬件双门禁共同 PASS
