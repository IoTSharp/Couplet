...
| CG-005 | 最新 `Tsdb.Generations` exact-revision lease 已接线；Couplet 已补 daemon watcher、database-root 单 owner、durable orderly-reopen cursor terminal cleanup 与真实 CLI commit 边界，仍缺真实进程重启/跨进程 root/cursor 竞争、cursor hard-kill CAS、真实双客户端和容量联合门禁 | SonnetDB M40 generation contract + Couplet CPL-013/CPL-015 | Couplet C1 发布及后续 | verifying | source lane 可执行 fenced publish/watch、active/retained/exact lease、durable cursor/cutoff cleanup、status/search/symbol；默认 package 仍仅 staging，不引入第二提交日志 |
...
- 实际：固定 package 只能完成 staging。source lane 已接通 filter-aware provenance、daemon watcher、planning KV + Document + FullText 原子发布、writer fence/cutoff cleanup及 status/search/symbol。database root 由 `.couplet-store.lock` 限制为单 live owner；fulltext cursor 使用持久 HMAC key、Available/Claimed CAS、默认两分钟绝对 TTL 和 128 slot，orderly store reopen 后通过 Core exact-revision lease 恢复同一 retired generation。terminal record 采用 version-CAS/snapshot/delete/snapshot，两个注入恢复窗口、timer、坏记录和 transition/release 故障均 fail closed。真实 CLI commit 前/后强杀仍只暴露完整旧/新 generation。真实进程重启/跨进程 root/cursor 竞争、cursor hard-kill CAS、真实双客户端、跨平台、随机故障和固定硬件容量仍未完成；既有 Medium/Large 数据来自全量 staging，不能外推 source generation 性能。
...
- 关闭证据：已有 Core exact-revision lease、Couplet active/retained/durable orderly-reopen cursor、database-root 单 owner、terminal cleanup 恢复窗口、watcher/provenance、cutoff cleanup、真实 CLI commit 边界和 status/search/symbol 回归；仍需真实进程重启/跨进程 root/cursor 竞争、cursor hard-kill CAS、真实 Codex/Claude Code 发布后查询一致性、跨平台/随机故障及 Medium/Large 固定硬件双门禁共同 PASS
...
- 实际：固定 3.1.0 package 仍复现并使用禁用 worker 的显式限制路径；最新 SonnetDB source 已修复 worker shutdown 并通过 Core win-x64 Native AOT 三 worker实跑。Couplet source win-x64 Native AOT CLI 首次 generation publish 与 unchanged no-op 均退出 0、无 CG-006 limitation；2026-08-31 本轮 CLI、Daemon、MCP Server 三个 source publish 均无未处置 IL/AOT warning且原生 `version` smoke 退出 0，生产 journey 与 7 天长稳尚待归档。
