...
- 关闭证据：已有 SonnetDB public API/Core 回归和 Couplet 小型 cursor/lease/cutoff cleanup、`workspace_status`、exact/fulltext Preview 与 `symbol_get` 回归；仍需 kill-before/after-publish、实时 watcher、query cursor、Codex/Claude Code 发布后查询一致性及 Medium/Large 固定硬件双门禁共同 PASS
