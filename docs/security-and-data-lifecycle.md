...
- 默认 3.1.0 package lane 没有 active generation 或 query lease，MCP 不读取 staging。source lane 由 `Tsdb.Generations` 原子发布 generation 独占 planning KV、Document 和 FullText 资源，并用 query lease 阻止 retired cleanup；`workspace_status`、`code_search` exact/fulltext Preview 与 `symbol_get` 均使用 per-request active lease，查询只访问该 lease 绑定的 path/FullText index。fulltext `code_search` cursor 只在 active generation 未切换时续页；cursor 不跨请求持有 retired lease，新 generation 发布后旧 cursor 显式 stale，不能借 cursor 读取已 retired 数据。qualified identity 歧义不静默选取；`symbol_get` cursor 仍 unavailable。
