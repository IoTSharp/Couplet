...
- 默认 3.1.0 package lane 没有 active generation 或 query lease，MCP 不读取 staging。source lane 由 `Tsdb.Generations` 原子发布 generation 独占 planning KV、Document 和 FullText 资源，并用 query lease 阻止 retired cleanup；`workspace_status` 与 `symbol_get` 使用 per-request active lease，`code_search` exact/fulltext Preview 使用 active、retained 或 exact-revision lease，查询只访问该 lease 绑定的 planning/path/FullText 资源。fulltext cursor 通过持久 HMAC key、`Available/Claimed` CAS registry、最多 128 slot 和默认两分钟绝对 TTL 跨请求保留 retired lease；新 generation 发布后续页仍固定原 revision，orderly store dispose/reopen 可恢复该 lease。未知 CAS、坏记录、timer 和 terminal cleanup 故障均 fail closed；成功 cleanup 或 dispose/reopen 才释放 lease，timer fault 后可能暂时保留。真实进程重启/跨进程竞争与 cursor hard-kill CAS 尚未验证。qualified identity 歧义不静默选取；`symbol_get` cursor 仍 unavailable。
...
- 默认 package Native AOT 因 CG-006 关闭 background workers 并暴露 limitation；最新 source lane 使用已修复的默认 worker。普通 source/JIT handshake 不声明 AOT 已验证；2026-08-31 source CLI、Daemon 和 MCP Server 的 win-x64 Native AOT publish 均无未处置 IL/AOT warning，三个原生 `version` smoke 退出 0。生产 index/watch/MCP journey、跨平台和 7 天长稳尚未归档，不能外推为 Production 维护证据。
