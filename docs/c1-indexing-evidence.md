...
| CPL-013 | frozen snapshot；added/modified/deleted/content-hash rename；producer upgrade full rebuild；debounced watcher | 原子 publish/branch-switch query visibility 阻塞 |
...
| CPL-015 | parse failure reason；cancellation；same-generation deterministic rebuild；index/count consistency；checkpoint/reopen manifest | publish crash/cursor/lease/cleanup 阻塞 |
...
写入后必须同时满足 Document count、FullText document count、Document index consistency 和 generation manifest validator。全部通过后才把 `GenerationState.Staging` manifest 写入 SonnetDB KV 并 checkpoint。内部探针冻结实际访问路径 `document_path_index:by_stable_id` 和 `document_fulltext:code_search`。
...
自动化覆盖 clean/dirty Git revision、嵌套构建产物、deny/ignore、path/symlink escape、Git 子进程取消、发现后文件突变、C#/TypeScript stable symbol 与 UTF-8 span、rename/modify/delete、producer upgrade、watch debounce，以及真实 SonnetDB staging/retry/reopen/access path。

2026-08-25 在 `329519d` 基线上的 C1 working tree 实测结果：Release build 为 0 warning/0 error，50/50 tests PASS；对 Couplet 自身执行 `workspace-scan` 得到 115 个候选、113 个 included files，随后 `index-stage` 成功写入 113 files、508 logical symbols、584 chunks 和 1205 FullText documents，`problems=0`。CLI、daemon 和 MCP Server 的 win-x64 Native AOT publish 均为 0 个未处置 IL/AOT warning，三个发布后的 executable 均能启动并报告固定 `SonnetDB.Core 3.1.0`。AOT CLI 单文件 staging 在 CG-006 配置门控下通过并显式报告后台维护 limitation。staging 报告按合同保持 `published=false`、`blocking_gap=CG-005`；烟测数据库在验证后删除，未作为仓库 artifact 保留。

## 未通过门禁

- Correctness/Recovery：缺 active generation 原子切换、query lease、kill-before/after-publish、cursor continuity、branch-switch visibility 和 lease-aware cleanup。
- Performance/Capacity：尚无冻结 Small/Medium/Large corpus 的 initial/incremental/exact/fulltext P50/P95/P99、RSS、allocation 与 I/O 报告。
...
- 双客户端合同：Codex/Claude Code 可读取既有 schema，但 `workspace_status`、`code_search`、`symbol_get` 在 generation 未发布时不能返回 staging 数据。
