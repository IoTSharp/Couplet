...
| CPL-014 | generation collection；stable ID/path indexes；FullText；512-record batch；source lane 原子发布 planning KV + Document + FullText；`workspace_status`、exact/fulltext `code_search`、fulltext 同 active cursor/no-scan 与 `symbol_get` per-request active lease | source `index-stage`、显式数据库 status/search/symbol 查询可用；跨 generation cursor 与 fulltext filter 仍不可用 |
