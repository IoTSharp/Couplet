...
C1 处于实现中，不是完成状态。CPL-010~012 已有可运行实现；CPL-013~015 在默认 `SonnetDB.Core 3.1.0` package lane 保留安全 staging，在显式 source lane 已接通最新 SonnetDB `Tsdb.Generations` 的 planning KV + Document + FullText 原子发布、active/retained lease/cursor、重开、no-op、writer fence、lease/cutoff-aware cleanup、真实子进程 commit 前后 kill/reopen、`workspace_status`、`code_search` exact/fulltext Preview、fulltext 同进程 retired cursor/no-scan、path/language/entity-kind 过滤计划与 `symbol_get` 小型回归。`CG-007` 的选择性 cleanup API/接线缺口已关闭，`CG-005` 仍为 verifying；实时 watcher freshness、进程重启后的 cursor 恢复、双客户端与固定硬件容量门禁仍未完成。固定 package 的 Native AOT 继续受 `CG-006` 阻塞；source lane 使用已修复的 worker 生命周期，本轮 CLI、Daemon 与 MCP Server 的 win-x64 Native AOT publish 均为 0 个未处置 IL/AOT warning，但 7 天长稳仍待归档。
...
| CPL-014 | generation collection；stable ID/path/language/entity-kind indexes；FullText；512-record batch；source lane 原子发布 planning KV + Document + FullText；`workspace_status`、exact/fulltext `code_search`、fulltext 同进程 active/retired cursor/no-scan、过滤式 posting 查询与 `symbol_get` query lease | source `index-stage`、显式数据库 status/search/symbol 查询可用；进程重启 cursor 仍不可用 |
| CPL-015 | parse/cancellation/marker/checkpoint/reopen；source publish/acquire、cursor revision、writer fence、lease/cutoff-aware cleanup；真实子进程提交前/后 kill/reopen 原子性；status 重开/selector fail closed | CG-007 API/接线已关闭；跨进程 cursor、实时 watcher 与容量发布证据未完成 |
...
`code_search` 每次调用持有单一 active 或 retained generation lease，并从该 lease 冻结的 collection/index 名称执行查询。exact 只按 stable ID 命中 `by_stable_id` path index；无过滤 fulltext 命中 `code_search` FullText index。带 path/language/entity-kind 的 fulltext 先在有界 planning snapshot 中匹配 path glob，再按 `by_path` 取得对应 document IDs；language/kind 分别走 `by_language` / `by_entity_kind`，候选集求交后调用 SonnetDB `SearchFullTextFiltered`，在 posting 处理阶段应用 allowed IDs。planning files、filter candidates 与 posting visits 共用单次查询访问预算；任一阶段耗尽均返回稳定 `budget_exhausted`，不返回部分排序结果。access path 如实报告 `planning_snapshot_path_glob`、各 Document path index 与 `document_fulltext_filtered:code_search`。fulltext 截断时签发绑定完整 path/language/kind query shape、generation/revision、little-endian offset 和随机 nonce 的 opaque cursor；首屏在查询前原子预留最多 128 个 slot，成功后以默认两分钟绝对 TTL 的进程内 lease 保留 generation，续页一次性接管并把同一到期时间传给下一个 cursor。最终页、取消、错误、到期、容量失败和 store dispose 都释放 lease/slot；单个 `TimeProvider` timer 按最早到期时间主动清理 idle lease。active 切换后，同一进程仍可读完 retained revision；进程重启、tamper、shape mismatch、负值/溢出均 fail closed。exact 最多返回一个唯一索引命中，不占 cursor slot。当前 FullText 没有原生 search-after，续页使用预算约束的 Top-K + offset，只 hydration 当前页；深页超过候选预算返回 `budget_exhausted`，不走 Document scan。`symbol_get` 的 stable ID 与带 language qualified identity 走唯一 `by_stable_id`，不带 language 的 qualified identity 走 `by_qualified_identity` 且最多取两条用于歧义检测。响应使用 source-generated typed DTO，携带 evidence、实际 access path、candidates/examined/returned、预算消耗和显式截断；symbol 歧义、非 symbol ID 与不完整记录 fail closed。`symbol_get` cursor 和进程重启后的 `code_search` 续页仍返回稳定 capability limitation；默认 package lane 不读取 staging。
...
自动化覆盖 clean/dirty Git revision、ignore/deny、path/symlink escape、Git 子进程取消、文件突变、三语言 stable symbol/chunk 与 UTF-8 span、rename/modify/delete、producer upgrade、watch、branch switch，以及 SonnetDB staging/retry/marker/reopen/access path。source lane 另覆盖 CLI 首次 publish、跨重开 active planning snapshot、unchanged no-op、缺新增索引时发布新 generation、连续 revision、可取消 writer fence、预取消 no-op 不清理、publish 后 cleanup fault 隔离、提交前/后确定性进程内故障重开、旧 lease 延迟 cleanup、租约释放后资源删除、旧 cursor stale，以及 zero/nonzero/UTC offset inclusive/mixed-age reopen/lease/cancellation/failure/最大 retention/连续六次 publish 的 cleanup 矩阵。`workspace_status` 回归覆盖空库、typed host success、调用前后 SonnetDB Document full-scan counter 不变、active revision 切换、旧 selector fail closed、重开、损坏 manifest、不泄露数据库路径、serialization-inclusive deadline 以及 stdio host 退出后数据库重开；`code_search` 回归覆盖 exact stable-ID path index、fulltext stdio typed 成功、path/language/entity-kind 单项与组合过滤、cursor shape、planning/filter/posting 三种预算失败、显式 access path、active lease 和默认 package gate。producer `1.0.0 -> 1.1.0` 当前仍只有 planner contract 回归，尚未执行真实旧 producer 数据库升级。
...
- Correctness/Recovery：**FAIL**。source runtime 已从 active planning snapshot 构造 plan，并实现 publish/no-op/reopen、writer fence、active/retained lease/cursor/cutoff cleanup、真实子进程 commit 前后 kill/reopen、`workspace_status`、exact/fulltext `code_search`、fulltext 同进程跨请求 retired cursor/no-scan、path/language/entity-kind 过滤计划与 `symbol_get` active-lease 回归；仍缺进程重启后的 cursor 恢复/稳定错误决策、实时 watcher freshness、branch-switch 查询语义和真实双客户端发布后会话。
...
- Native AOT lifecycle：默认 3.1.0 package 仍关闭不兼容 worker并报告 CG-006；本轮 source CLI、Daemon 与 MCP Server 的 win-x64 publish 均为 0 个未处置 IL/AOT warning，CLI 的 2026-08-29 publish/no-op runtime smoke 仍是当前最新实跑，7 天长稳仍未归档。
- 双客户端合同：Codex/Claude Code 的能力门控已验证；source lane `workspace_status`、fulltext `code_search`、同进程 active/retired cursor 与 `symbol_get` 已有 stdio/typed 小型成功回归，但尚无真实双客户端发布后会话证据，进程重启 cursor 仍 unavailable。

关闭 C1 还需完成实时 watcher 与并发协调、进程重启 MCP cursor 决策、删除/重命名/branch switch 查询语义、真实双客户端、固定硬件容量和 7 天增长联合回归。不得以应用层第二提交日志、直接查询 package staging、吞 dispose 异常或隐藏 maintenance 状态绕过。
