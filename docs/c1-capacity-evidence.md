...
- 仓库内 Codex 与 Claude Code client-profile/合同 fixture 的 MCP initialize/tools-call 回归证明 `workspace_status`、`code_search`、`symbol_get` 均返回 `CG-005/generation_publish_blocked`，响应不包含 staging items；该回归未启动真实客户端进程。

这次 2026-08-25 容量运行没有覆盖 active publish 点 kill-before/after、query lease、cursor continuity 或 retired cleanup。后续 source-lane 小型回归已覆盖 active/retained/exact-revision lease、database-root 单 live owner、持久 registry 支持的 orderly store reopen cursor terminal cleanup、cutoff-aware cleanup，以及真实 `Couplet.Cli` 子进程在 commit 前后强杀/重开的原子可见性；这些结果没有在本文的 Medium/Large 固定语料上复测，也不提供真实进程重启/跨进程 root/cursor 竞争、cursor hard-kill CAS、随机进程故障、真实双客户端、容量或长稳证据。因此 CG-005 继续为 `verifying`，C1 Performance/Capacity gate 继续为 **FAIL**；默认 package 的 Native AOT background maintenance 仍由 CG-006 阻塞。
